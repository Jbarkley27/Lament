...
public abstract class EnemyBase : MonoBehaviour
{
    ...
    public float averageCastDistance = 80f;
    ...
    protected virtual void Update()
    {
        ...
        if (playerScentNode != Vector3.zero && currentState == EnemyState.Seeking)
        {
    ...
    protected virtual void EngagingState()
    {
        ...
        Logger.Log($"Enemy {name} coroutine is null: {attackCoroutine == null} isAttacking: {isAttacking} PlayerInAttackRange: {PlayerInAttackRange()} CanAttack: {CanAttack}");
        if (attackCoroutine == null && !isAttacking && PlayerInAttackRange() && CanAttack)
        {
    ...
    public IEnumerator InitiateAttack()
    {
        ...
        yield return new WaitForSeconds(2f); // small delay before next possible attack
        CanAttack = true;
    ...
    private void MoveRigidbody()
    {
        ...
        if (IsStuck)
        {
            avoidance = ComputeEscapeForce();
        }
        else
        {
            avoidance = ComputeAvoidanceForce();
        }

        avoidance.y = 0;

        Vector3 steering = (desiredDirection + avoidance).normalized;

        Vector3 targetVelocity = steering * moveSpeed;
        currentVelocity = Vector3.Lerp(currentVelocity, targetVelocity, steerSpeed * Time.fixedDeltaTime);
        Vector3 finalVel = Vector3.MoveTowards(rb.linearVelocity, currentVelocity, acceleration * Time.fixedDeltaTime);
        rb.AddForce(finalVel, ForceMode.Force);
    }
    ...
    private Vector3 ComputeAvoidanceForce()
    {
        ...
        {
            float t = raysPerCone == 1 ? 0.5f : (float)i / (raysPerCone - 1);
            float angle = Mathf.Lerp(-halfCone, halfCone, t);
            Vector3 rayDir = Quaternion.Euler(0, angle, 0) * transform.forward;

            float weight = 1f - Mathf.Abs(angle) / halfCone;
            Vector3 rayOrigin = transform.position + rb.linearVelocity * lookaheadTime + transform.forward * 0.2f;

            bool rayHitSomething = false;   // <-- track hit per ray
            float nearestHitDistance = avoidanceCastDistance;
            ...
            foreach (var layer in avoidanceLayers)
            {
                                    ...
                                    layer.layerMask))
                {
                    ...
                    float distanceFactor = 1f - (hit.distance / avoidanceCastDistance);
            ...
            // ---- Debug drawing (only once per ray) ----
            if (debugMode)
            {
                if (rayHitSomething)
                    Debug.DrawRay(rayOrigin, rayDir * nearestHitDistance, Color.red);
                else
                    Debug.DrawRay(rayOrigin, rayDir * avoidanceCastDistance, Color.green);
            }
        }
    ...
    public virtual void OnSpawned()
    {
        ...
        attackRange = Random.Range(attackRange * 0.8f, attackRange * 1.2f) + 15f;
    }
    ...
    public void RandomizeStats()
    {
        ...
        avoidanceCastDistance = Random.Range(averageCastDistance * 0.8f, averageCastDistance * 1.2f);
